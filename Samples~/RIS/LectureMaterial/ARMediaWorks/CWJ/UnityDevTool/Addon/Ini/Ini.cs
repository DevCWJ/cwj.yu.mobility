...
public struct IniValue
{
    ...
    public bool TryConvertByte(out byte result)
    {
        ...
        if (TryParseByte(Value.Trim(), out result))
        {
            return true;
        }
        return false;
...
public class IniFile : IEnumerable<KeyValuePair<string, IniSection>>, IDictionary<string, IniSection>
{
    ...
    private const string MultiLineTag = "@MultiLine@";
    ...
    public void SetMultiLineValue(string section, string key, string value)
    {
        value = value.Replace(SpaceLine, "\n");
        value = value.TrimEnd('\n');
        value = Indent + value.Replace("\n", (SpaceLine + Indent));
        this[section][key] = $"{MultiLineTag}{SpaceLine}{MultiLineStart}{SpaceLine}{value}{SpaceLine}{MultiLineEnd}";
    }

    public void Save(string path, FileMode mode = FileMode.Create, bool isOpenViaNotepad = false)
    {
        using (var stream = new FileStream(path, mode, FileAccess.Write))
        {
            Save(stream);
            if (isOpenViaNotepad && Application.isEditor)
            {
    ...
    List<string> multiLineValues = new List<string>();
    ...
    public void Load(StreamReader reader, bool ordered = false)
    {
        ...
        while (!reader.EndOfStream)
        {
            ...
            if (!isMultiLineValue)
            {
                ...
                if (trimStart.Length == 0) continue;
                ...
                {
                    isMultiLineValue = trimStart.EndsWith(MultiLineTag);
                    string key;
                    IniValue val;
                    if (LoadValue(line, out key, out val))
                    {
                        if (isMultiLineValue)
                        {
                            ...
                            multiLineValues.Add(MultiLineTag);
                        }
                        else
                        {
                            section[key] = val;
            ...
            {
                multiLineValues.Add(line);
                if (line.Equals(MultiLineEnd))
                {
                    section[multilineKey] = new IniValue(string.Join("\r\n", multiLineValues));
