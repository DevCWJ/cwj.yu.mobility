...
/// <summary>
/// Runtime NavMesh bake 와 동시에 호환되도록 수정함
/// <para/>[20.06.17]
/// </summary>
public class NavMeshCleaner : MonoBehaviour
{
    public bool HasRuntimeNavMesh()
    {
...
#if CWJ_EXISTS_RUNTIMENAVMESH
    [Readonly, FindObject(isIncludeInactive: false), HideConditional(nameof(HasRuntimeNavMesh))] public NavMeshSurface navMeshSurface;
#endif
    ...
    [SerializeField, Readonly] private GameObject replicaSourceObj;
    ...
    List<GameObject> tmpMeshObjList = new List<GameObject>();
    ...
    {
        if (replicaSourceObj == null)
        {
            replicaSourceObj = new GameObject("ReplicaSource(Created)", typeof(MeshFilter), typeof(MeshRenderer));
            replicaSourceObj.hideFlags = HideFlags.NotEditable | HideFlags.HideInHierarchy;
            replicaSourceObj.transform.SetParentAndReset(transform);
            replicaSourceObj.SetActive(false);
        }

#if CWJ_EXISTS_RUNTIMENAVMESH
        if (HasRuntimeNavMesh())
        {
            replicaSourceObj.GetOrAddComponent<MeshCollider>();
            var modifier = replicaSourceObj.GetOrAddComponent<NavMeshModifier>();
            modifier.overrideArea = true;
            modifier.area = 1;
        }
#endif

        replicaSourceObj.gameObject.layer = gameObject.layer;
        replicaSourceObj.tag = gameObject.tag;

        MeshRenderer meshrenderer = replicaSourceObj.GetComponent<MeshRenderer>();
        meshrenderer.sharedMaterial = AssetDatabase.GetBuiltinExtraResource<Material>("Default-Diffuse.mat");
        GameObjectUtility.SetStaticEditorFlags(replicaSourceObj, GameObjectUtility.GetStaticEditorFlags(gameObject) | StaticEditorFlags.NavigationStatic);
        GameObjectUtility.SetNavMeshArea(replicaSourceObj, 1);
    }
...
#endif
        int meshObjListCnt = tmpMeshObjList.Count;
        for (int i = 0; i < meshObjListCnt; i++)
        {
#if UNITY_EDITOR
            Undo.DestroyObjectImmediate(tmpMeshObjList[i]);
#else
            Destroy(tmpMeshObjList[i]);
#endif
        }
        tmpMeshObjList.Clear();
    ...
    {
        int tmpMeshObjListCnt = tmpMeshObjList.Count;
        for (int i = 0; i < tmpMeshObjListCnt; i++)
            tmpMeshObjList[i].SetActive(visible);
#if UNITY_EDITOR
        SceneView.RepaintAll();
#endif
    }
    ...
    public bool HasMesh()
    {
        return tmpMeshObjList.Count != 0 ? true : false;
    }

    public bool MeshVisible()
    {
        if (tmpMeshObjList.Count > 0)
            return tmpMeshObjList[0].activeSelf;
        return false;
    }

    public void BuildMesh()
    {
        Mesh[] meshs = CreateMesh();
#if UNITY_EDITOR
        Undo.RegisterCreatedObjectUndo(this, "build");
#endif
        for (int i = 0; i < meshs.Length || i == 0; i++)
        {
            GameObject obj;
            if (i >= tmpMeshObjList.Count)
            {
                obj = Instantiate<GameObject>(replicaSourceObj, transform.position, transform.rotation, transform);
                obj.SetActive(true);
                obj.name = gameObject.name + "_Mesh(DontSave)";
                obj.hideFlags = HideFlags.DontSave;

                tmpMeshObjList.Add(obj);
#if UNITY_EDITOR
            ...
            {
                obj = tmpMeshObjList[i].gameObject;
            }

            obj.hideFlags = i == 0 ? (HideFlags.DontSave | HideFlags.HideInHierarchy) : tmpMeshObjList[0].hideFlags;

            MeshFilter meshfilter = tmpMeshObjList[i].GetComponent<MeshFilter>();
#if UNITY_EDITOR
            Undo.RecordObject(meshfilter, "MeshUpdate");
#endif
            meshfilter.sharedMesh = meshs.Length == 0 ? null : meshs[i];

            if (HasRuntimeNavMesh())
            {
                var meshCol = tmpMeshObjList[i].GetComponent<MeshCollider>();
                if (meshCol != null) meshCol.sharedMesh = meshfilter.sharedMesh;
            }
        }

        while (tmpMeshObjList.Count > meshs.Length)
        {
#if UNITY_EDITOR
            Undo.DestroyObjectImmediate(tmpMeshObjList[tmpMeshObjList.Count-1]);
#else
            Destroy(tmpMeshObjList[tmpMeshObjList.Count - 1]);
#endif
            tmpMeshObjList.RemoveAt(tmpMeshObjList.Count-1);
        }
    ...
    Mesh[] CreateMesh()
    {
...
#if UNITY_EDITOR
            if ((i % 100) == 0)
                EditorUtility.DisplayProgressBar("Export Nav-Mesh (Phase #1/3)", $"Weld Vertex {i}/{table.Length}", Mathf.InverseLerp(0, table.Length, i));
#endif
        }
        ...
        Vector3[] walkpoint = walkablePoints.ToArray();
    ...
    {
        NavMeshCleaner navMeshCleaner;

        void OnEnable()
        {
            navMeshCleaner = (NavMeshCleaner)target;
        ...
        {
#if CWJ_EXISTS_RUNTIMENAVMESH
            EditorGUILayout.HelpBox("[NavMeshSurface를 사용하는경우]" +
                                    //"\n0. Bake되는 오브젝트들의 Mesh를 isRedable true가 되어야함" + -> MeshUtil.SetReadableMesh로 자동해결
                                    "\n1. NavMeshSurface Bake" +
                                    "\n2. Calculate 클릭(NavMeshSurface 오브젝트가 활성화되어있는 상태여야함)" +
                                    "\n3. Show Mesh로 Mesh를 보이게한 후 NavMeshSurface Bake" +
                                    "\n4. Hide Mesh", MessageType.Info);
#endif
            EditorGUILayout.HelpBox(m_OverPoint != -1 ? "Press Control and click to remove the point." : "Press Control and click to add a walkable point.", navMeshCleaner.walkablePoints.Count == 0 ? MessageType.Error : MessageType.Info);

            base.OnInspectorGUI();

            NavMeshCleaner t = (NavMeshCleaner)target;

            if (t.tmpMeshObjList.Count > 0)
            {
                EditorGUI.BeginChangeCheck();
                bool hideInHierarchy = EditorGUILayout.Toggle("Hide Temp Mesh Object In Hierarchy", (t.tmpMeshObjList[0].hideFlags & HideFlags.HideInHierarchy) != 0 ? true : false);
                if (EditorGUI.EndChangeCheck())
                {
                    t.replicaSourceObj.hideFlags = hideInHierarchy ? (t.replicaSourceObj.hideFlags | HideFlags.HideInHierarchy) : (t.replicaSourceObj.hideFlags & (~HideFlags.HideInHierarchy));

                    for (int i = 0; i < t.tmpMeshObjList.Count; i++)
                        t.tmpMeshObjList[i].hideFlags = hideInHierarchy ? (t.tmpMeshObjList[i].hideFlags | HideFlags.HideInHierarchy) : (t.tmpMeshObjList[i].hideFlags & (~HideFlags.HideInHierarchy));
                    try
                    {
            ...
            if (t.HasMesh() && GUILayout.Button("Reset WalkablePoints", GUILayout.Height(30.0f)))
            {
                ...
                navMeshCleaner.walkablePoints.Clear();
                SceneView.RepaintAll();
            }
        ...
        void OnSceneGUI()
        {
            ...
            {
                // draw
                for (int i = 0; i < navMeshCleaner.walkablePoints.Count; i++)
                {
                    Vector3 pos = navMeshCleaner.transform.TransformPoint(navMeshCleaner.walkablePoints[i]);
                    float unitsize = WorldSize(1.0f, sceneview.camera, pos);

                    Handles.color = Color.black;
                    DrawDisc(pos, Vector3.up, unitsize * 15);

                    Handles.color = i == m_OverPoint ? Color.red : Color.green;
                    Handles.DrawSolidDisc(pos, Vector3.up, unitsize * 10);
                    Handles.DrawLine(pos, pos + Vector3.up * (unitsize * 200.0f));
                }
            }
        ...
        void MouseEvent(EventType type, Vector2 mouseposition, bool controldown)
        {
            ...
            if (type == EventType.MouseMove)
            {
                ...
                for (int i = 0; i < navMeshCleaner.walkablePoints.Count; i++)
                {
                    Vector3 p = navMeshCleaner.transform.TransformPoint(navMeshCleaner.walkablePoints[i]);
            ...
            if (type == EventType.MouseDown && controldown == true)
            {
                if (m_OverPoint != -1)
                {
                    Undo.RecordObject(navMeshCleaner, "Remove Point");
                    navMeshCleaner.walkablePoints.RemoveAt(m_OverPoint);
                    m_OverPoint = -1;
                }
                else
                {
                    ...
                    {
                        Undo.RecordObject(navMeshCleaner, "Add Point");
                        NavMeshHit navMeshHit;
                        Vector3 hitWorldPos = navMeshCleaner.transform.InverseTransformPoint(hit.point);
                        if (!navMeshCleaner.walkablePoints.Contains(hitWorldPos) && NavMesh.SamplePosition(hitWorldPos, out navMeshHit, 1, 1))
                        {
                            if (navMeshHit.position.x == hitWorldPos.x && navMeshHit.position.z == hitWorldPos.z)
                            {
                                navMeshCleaner.walkablePoints.Add(hitWorldPos);
                            }
                        }
                    }
                    else
                    {
                        ...
                        if (mint < 1000.0f)
                        {
                            ...
                            navMeshCleaner.walkablePoints.Add(navMeshCleaner.transform.InverseTransformPoint(point));
                        }
                    }
                }
                HandleUtility.Repaint();
