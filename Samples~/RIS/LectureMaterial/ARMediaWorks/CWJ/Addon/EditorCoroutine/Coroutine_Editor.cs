    ...
    public class Coroutine_Editor
    {
        private struct YieldProcessor
        {
            ...
            public void Set(object yield)
            {
                ...
                if (type == typeof(WaitForSeconds_Editor))
                {
                    targetTime = EditorApplication.timeSinceStartup + (yield as WaitForSeconds_Editor).WaitTime;
                    dataType = DataType.WaitForSeconds;
                }
                else if (type == typeof(Coroutine_Editor))
                {
            ...
            public bool MoveNext(IEnumerator enumerator)
            {
                ...
                switch (data.type)
                {
                    ...
                    case DataType.EditorCoroutine:
                        advance = (data.current as Coroutine_Editor).m_IsDone;
                        break;
        ...
        internal Coroutine_Editor(IEnumerator routine)
        {
            m_Owner = null;
            m_Routine = routine;
        ...
        internal Coroutine_Editor(IEnumerator routine, object owner)
        {
