        ...
        public class Scenario
        {
            ...
            public void Enable(Topic topic)
            {
                ...
                if (activeSyncTrfs.LengthSafe() > 0)
                    activeSyncTrfs.ForEach(t => { if (t && !t.gameObject.activeSelf) t.gameObject.SetActive(true); });
                if (activeSyncMonoBehavs.LengthSafe() > 0)
                    activeSyncMonoBehavs.ForEach(m => { if (m && !m.enabled) m.enabled = true; });
                topic.topicUI.SetTarget(rotateTargetTrf);
            ...
            public void Disable(Topic topic, bool isForAllInit)
            {
                ...
                if (activeSyncTrfs.LengthSafe() > 0)
                    activeSyncTrfs.ForEach(t => { if (t && t.gameObject.activeSelf) t.gameObject.SetActive(false); });

                if (activeSyncMonoBehavs.LengthSafe() > 0)
                    activeSyncMonoBehavs.ForEach(m => { if (m && m.enabled) m.enabled = false; });
            ...
            public void Init(Topic topic)
            {
                ...
                void CheckAndAddCollider(Transform target)
                {
                    ...
                    if (target && !target.GetComponent<Collider>())
                    {
                        ...
                        col.isTrigger = true;
                        col.radius = 0.015f; col.center = Vector3.zero;
                    }
                }
