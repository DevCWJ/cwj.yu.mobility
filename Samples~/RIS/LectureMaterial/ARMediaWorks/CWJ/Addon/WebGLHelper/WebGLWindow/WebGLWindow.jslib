var WebGLWindow = {
    ...
    ExitFullscreen : function() {
        ...
        // 전체화면 해제 후 강제로 캔버스 크기 리셋
        /*var canvas = document.getElementsByTagName('canvas')[0];
        if (canvas) {
            // 초기 크기를 기억하고 있으면 그 값으로 되돌리기
            canvas.style.width = "auto";
            canvas.style.height = "auto";
        }
    
        // 부모 요소의 ID 복구
        var fullscreenRoot = document.getElementById("WebGLWindow:Canvas:ParentNode");
        if (fullscreenRoot) {
            fullscreenRoot.style.width = "auto";
            fullscreenRoot.style.height = "auto";
        }*/
    },
    ...
    GoUrlWithoutHistory : function (str) {
        window.location.replace(Pointer_stringify(str));
    },
    GoUrlWithHistory : function (str) {
        window.location.href = Pointer_stringify(str);
    },
    OpenUrlWithNewPopup : function (str) {
