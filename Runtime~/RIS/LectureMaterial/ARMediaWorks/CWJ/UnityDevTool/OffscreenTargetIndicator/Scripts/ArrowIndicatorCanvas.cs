...
namespace CWJ
{
	public class ArrowIndicatorCanvas : ArrowIndicatorAbstract
	{
		...
		private void FadingDownValues()
		{
			...
			{
				transform.localScale = Vector3.Lerp(Vector3.one, Vector3.zero, elapsedTime / indicator.transitionDuration);
			}
		}
		...
		private void FadingUpValues()
		{
			...
			if (indicator.transition == IndicatorSetting.Transition.Scaling)
			{
				transform.localScale = Vector3.Lerp(Vector3.zero, Vector3.one, (elapsedTime - indicator.transitionDuration) / indicator.transitionDuration);
			}
		}
