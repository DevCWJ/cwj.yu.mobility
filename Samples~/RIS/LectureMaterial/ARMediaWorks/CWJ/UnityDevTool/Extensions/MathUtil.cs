...
namespace CWJ
{
    ...
    {
        public static int[] GetRandomIndex(int length, int min, int max)
        {
            int randomRange = max - min;
            List<int> randomList = new List<int>(randomRange);
            for (int i = 0; i < randomRange; i++)
            {
                randomList.Add(i + min);
            }

            int[] result = new int[length];
            for (int i = 0; i < length; i++)
            {
                int listIndex = UnityEngine.Random.Range(0, randomList.Count);
                result[i] = randomList[listIndex];
                randomList.RemoveAt(listIndex);
            }
            return result;
        }
        ...
        /// <returns></returns>
        public static bool IsTargetInSightAngle(this Camera camera, Bounds targetBounds)
        {
        ...
        /// <param name="isClamping"></param>
        /// <returns></returns>
        public static float ConvertOtherRangeValue((float min, float max) range1, float value1, (float min, float max) range2, bool isClamping = true)
        {
            if (isClamping)
            {
                if(range1.min > range1.max)
                {
                    float tmp = range1.min;
                    range1.min = range1.max;
                    range1.max = tmp;
                }
                if (range2.min > range2.max)
                {
                    float tmp = range2.min;
                    range2.min = range2.max;
                    range2.max = tmp;
                }
                //throw new ArgumentException($"'{nameof(range1)}'({range1.mmin}~{range1.max}) or '{nameof(range2)}'({range2.min}~{range2.max}) is not verified");
            }

            float minMaxDiff1 = range1.max - range1.min;

            float minMaxDiff2 = range2.max - range2.min;
            if (minMaxDiff1 == 0 || minMaxDiff2 == 0)
            {
                return range2.min;
            }
            ...
            if (isClamping)
            {
                value1 = Mathf.Clamp(value1, range1.min, range1.max);
            }
            else
            {
                if (value1 < range1.min || value1 > range1.max)
                {
                    throw new ArgumentException("value1 is out of range");
                }
            }
            ...
            else if (value1 == range1.max)
            {
                return range2.max;
            }

            float minOffset1 = value1 - range1.min;

            return (minOffset1 / minMaxDiff1 * minMaxDiff2) + range2.min;
        }
        ...
        /// <returns></returns>
        public static float GetNearNumber(IList<float> list, float target)
        {
            float min = Math.Abs(list[0] - target);
            float nearData = list[0];

            int listCnt = list.Count;

            for (int i = 1; i < listCnt; i++)
            {
                float a = Math.Abs(list[i] - target);
                if (min > a)
                {
                    min = a;
                    nearData = list[i];
        ...
        public static int GetNearNumberInt(IList<int> list, int target)
        {
            int min = Math.Abs(list[0] - target);
            int nearData = list[0];

            int listCnt = list.Count;
            for (int i = 1; i < listCnt; i++)
            {
                int a = Math.Abs(list[i] - target);
                if (min > a)
                {
                    min = a;
                    nearData = list[i];
        ...
        public static short Clamp(short input, short min, short max)
        {
        ...
        public static bool Clamp(ref float refValue, float min, float max)
        {
            if (min > refValue)
            {
                refValue = min;
        ...
        /// <returns></returns>
        /// <remarks></remarks>
        public static short ClampOrDefault(short input, short min, short max, short defaultValue)
        {
            return input < min || input > max ? defaultValue : input;
        }

        public static int ClampOrDefault(int input, int min, int max, int defaultValue)
        {
            return input < min || input > max ? defaultValue : input;
        }

        public static long ClampOrDefault(long input, long min, long max, long defaultValue)
        {
            return input < min || input > max ? defaultValue : input;
        }

        public static float ClampOrDefault(float input, float min, float max, float defaultValue)
        {
            return input < min || input > max ? defaultValue : input;
        }
        ...
        #endregion "Simple fuzzy arithmetic"
        ...
        #endregion Geometric Calculations
